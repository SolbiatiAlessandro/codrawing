...
The score-aware policy keeps a small private experimental memory across otherwise stateless model calls: recent scores, best score, last pixel, and whether that pixel was accepted or collided. Seat 5 is the score captain and must publish an observe/retain-or-reject/direct cycle; the other seats must cite the delta and follow its concrete directives. A non-white write remains locked to the seat's assigned color, while `#FFFFFF` is available as an eraser so a harmful experiment can actually be reverted.
